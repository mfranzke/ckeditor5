...
class ContextFactory {
	...
	public updateRelation( opA: Operation, opB: Operation ): void {
		...
		if ( opA instanceof MoveOperation ) {
			...
			if ( opB instanceof MergeOperation ) {
				// We can only have one relation set for each pair, but the cases here are okay here. There is one case when a pair
				// can fulfill two cases -- `mergeSameTarget` and `mergeSameElement` if operations are equal. But in this case it is
				// okay that `mergeSameElement` overwrites `mergeSameTarget`.
...
setTransformation( MergeOperation, MergeOperation, ( a, b, context ) => {
	...
	if ( a.targetPosition.isEqual( b.targetPosition ) ) {
		...
		//
		// Usually, when operations point to the same merge target, the operations are equal. However, in some more complex undo cases
		// (including multiple steps) it happens that two different elements are merged into the same element. It only happens "temporarily"
		// as the operation is being transformed during undo process. It doesn't seem that this can happen in real-time collaboration.
		//
